...
const Slotify = (superclass) => class extends superclass {
  constructor() {
    super();
      ...
      const SSlot = class extends HTMLElement {
        ...
        updateAssignedContent(checkForEmptySlot=false) {
          ...
          const unplacedNodes = Array.from(lightDOM.childNodes).filter(node => {
            return node.parentNode === this.sRoot.parentElement && node !== this.sRoot; // return all nodes outside the <s-root>, they haven't been moved yet
          });
          let content = [];
...
class SlotifyTestComponent extends Slotify(LitElement) {
  render() {
    return html`
